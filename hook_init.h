...
public:
	...
	void AddWithHash(const KeyT &key, tjs_uint32 hash, const ValueT &value)
	{
		...
		// insert after lv1
		element *newelm = (element *)TVP_malloc(sizeof(element));
		newelm->Flags = 0;
...
private:
	...
	void DeleteByElement(element *elm)
	{
		...
		if (!(elm->Flags & TJS_HS_HASH_LV1)) TVP_free (elm);
	}
...
class Hooker {
	bool req_interrupt;
	static void err_reporter(enum x86_report_codes code, void *arg, void *junk);
	static bool hook_fail();
public:
	Hooker();
