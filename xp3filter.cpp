...
static DWORD xcode_rand(struct cxdec_xcode_status *xcode)
{
...
static int push_dwordxcode(struct cxdec_xcode_status *xcode, DWORD code)
{
	if ((DWORD)xcode->curr - (DWORD)xcode->start + 4 > xcode->space_size)
		return 0;
...
tjs_error CBinaryAccessor::FuncXor(tjs_int numparams, tTJSVariant **param)
{
	...
	if (len>16)
	{
		unsigned long k = 0x1010101 * xorval;
		for (i = 0; i < ((unsigned int)buf & 3); i++) buf[i] ^= xorval;
		len -= i;
		unsigned long *dbuf = (unsigned long*)(buf + i);
		for (i = 0; i < len >> 2; i++) *(dbuf++) ^= k;
		buf = (unsigned char*)dbuf;
		for (i = 0; i < (len & 3); i++) *buf++ ^= xorval;
	} else
		for (i = 0; i < len; ++i)
			buf[i] ^= xorval;
	return TJS_S_OK;
...
static tTJSCriticalSection sTVPScriptEngineStackLock;
...
static void AddXP3Decoder() {
	...
	iTJSDispatch2 *host_global = TVPGetScriptDispatch();
#define REGISTER_OBJECT(classname) \
	host_global->PropGet(0, TJS_W(#classname), nullptr, &val, host_global); \
	dsp = val.AsObjectNoAddRef(); \
    val = tTJSVariant(dsp/*, dsp*/); \
    dsp->Release(); \
    global->PropSet(TJS_MEMBERENSURE|TJS_IGNOREPROP, TJS_W(#classname), NULL, \
    &val, global);
	REGISTER_OBJECT(Debug);
	REGISTER_OBJECT(System);
	REGISTER_OBJECT(Storages);
	TJSNativeClassRegisterNCM((tTJSNativeClass*)dsp, TJS_W("setXP3ArchiveExtractionFilter"),
        new XP3FilterRegister(&decoder),
        TJS_W("Storages"), nitMethod, TJS_STATICMEMBER);
    ...
    TVPXP3ArchiveExtractionFilterWrapper(tTVPXP3ExtractionFilterInfo *info)
{
    ...
    XP3FilterDecoder decoder;
    {
        tTJSCSH lock(sTVPScriptEngineStackLock);
